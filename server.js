...
const VEXO_51_PRODUCTS = [
  { id:'vexo-digital-auto-delivery', name:'VEXO 디지털 자동지급팩', category:'봇 옵션', price:19900, badge:'DELIVERY PRO', description:'구매 완료 후 디지털 상품을 빠르게 전달할 수 있도록 자동 지급 흐름을 확장하는 영구 기능팩입니다.', features:['디지털 상품 자동 지급 흐름','다운로드 횟수 제한 옵션','재다운로드 지원','지급 이력 기록','지급 완료 처리 연동','1회 구매 후 영구 적용'] },
  { id:'vexo-alert-automation', name:'VEXO 알림 자동화팩', category:'봇 옵션', price:9900, badge:'ALERT', description:'주문·입금확인·문의·지급 완료 같은 운영 이벤트를 한눈에 확인하고 빠르게 알릴 수 있는 영구 알림 확장팩입니다.', features:['새 주문 알림','입금 확인 요청 알림','문의 도착 알림','지급 완료 알림','Discord 웹훅 알림 연동','1회 구매 후 영구 적용'] },
  { id:'vexo-seller-dashboard', name:'VEXO SELLER DASHBOARD', category:'패키지', price:29900, badge:'DASHBOARD', description:'판매자가 주문·매출·상품 성과를 한 화면에서 확인할 수 있도록 운영 대시보드를 강화하는 영구 패키지입니다.', features:['실시간 주문 현황','일/월 매출 요약','상품별 판매량·매출','진행 단계별 주문 분석','최근 운영 활동 위젯','CSV 운영 데이터 내보내기','1회 구매 후 영구 적용'] }
];
const existing51 = new Set(db.products.map(p => p.id));
for (const p of VEXO_51_PRODUCTS) {
...
function migrateBrandText() {
  const LEGACY_BRAND = ['VE', 'OX'].join('');
  const rename = value => typeof value === 'string' ? value.replaceAll(LEGACY_BRAND, 'VEXO') : value;
  for (const p of db.products) {
    if (p.name) p.name = rename(p.name);
    if (p.description) p.description = rename(p.description);
    if (p.badge) p.badge = rename(p.badge);
    if (Array.isArray(p.features)) p.features = p.features.map(rename);
  }
  for (const o of db.orders) if (o.productName) o.productName = rename(o.productName);
  for (const log of db.purchaseLogs) if (log.productName) log.productName = rename(log.productName);
  for (const a of db.announcements) {
    if (a.title) a.title = rename(a.title);
    if (a.text) a.text = rename(a.text);
  }
  for (const i of db.inquiries) {
    if (i.subject) i.subject = rename(i.subject);
    if (Array.isArray(i.messages)) {
      for (const m of i.messages) {
        if (m.senderName) m.senderName = rename(m.senderName);
        if (m.text) m.text = rename(m.text);
      }
    }
  }
  if (db.settings) {
    if (db.settings.siteName) db.settings.siteName = rename(db.settings.siteName);
    if (db.settings.notice) db.settings.notice = rename(db.settings.notice);
  }
}
migrateBrandText();
...
// VEXO permanent revenue products: no subscription / no maintenance period.
// Retire the earlier setup and UPDATE PASS SKUs so they disappear from the catalog on upgrade.
const RETIRED_VEXO_PRODUCT_IDS = new Set([
  'vexo-service-bot-setup',
  'vexo-service-server-premium-setup',
  'vexo-maintenance-pass-1m',
  'vexo-maintenance-pass-6m'
]);
db.products = db.products.filter(p => !RETIRED_VEXO_PRODUCT_IDS.has(String(p.id)));
db.products = db.products.filter(p => String(p.id) !== 'vexo-multiserver' && String(p.name) !== 'VEXO 멀티서버 관리팩');

const VEXO_UPGRADE_PRODUCTS = [
  { id:'vexo-feature-ticket-automation', name:'VEXO 티켓 자동화 확장팩', category:'봇 옵션', price:9900, badge:'TICKET', description:'주문·문의 티켓의 반복 작업을 줄이고 처리 흐름을 더 빠르게 만드는 영구 기능 확장팩입니다.', features:['티켓 자동 제목·안내 문구 옵션','처리 상태 안내 개선','자동 닫기/정리 옵션','주문자 접근 보호','1회 구매 후 영구 적용'] },
  { id:'vexo-feature-payment-automation', name:'VEXO 결제 자동화 확장팩', category:'봇 옵션', price:14900, badge:'PAYMENT', description:'입금확인 요청과 결제 상태 전달을 더 체계적으로 관리하는 영구 기능 확장팩입니다.', features:['입금확인 요청 상태 관리','관리자 결제 알림 강화','결제 상태 임베드 개선','미처리 주문 확인 보조','1회 구매 후 영구 적용'] },
  { id:'vexo-feature-operations-automation', name:'VEXO 운영 자동화 확장팩', category:'봇 옵션', price:19900, badge:'AUTO', description:'반복 운영 업무를 줄이는 실전형 자동화 기능을 묶은 영구 확장팩입니다.', features:['주문 상태 자동화 보조','운영 알림 강화','자동 만료·정리 옵션','주기적 데이터 백업 보조','1회 구매 후 영구 적용'] },
  { id:'vexo-feature-security', name:'VEXO 보안 강화팩', category:'봇 옵션', price:14900, badge:'SECURITY', description:'권한 분리와 운영 영역 보호를 강화하는 영구 보안 옵션입니다.', features:['관리 기능 접근 보호','위험 권한 차단 보조','스태프 영역 보호','주요 작업 감사 로그 강화','1회 구매 후 영구 적용'] },
  { id:'vexo-feature-statistics', name:'VEXO 통계 대시보드팩', category:'봇 옵션', price:12900, badge:'STATS', description:'판매·주문·매출을 한눈에 확인할 수 있도록 운영 통계를 확장하는 영구 옵션입니다.', features:['주문량 통계','완료 매출 통계','평균 주문액','진행중 주문 현황','1회 구매 후 영구 적용'] },
  { id:'vexo-custom-lite', name:'VEXO 간단 커스텀 옵션', category:'커스텀', price:19900, badge:'LITE', description:'기존 VEXO 봇의 작은 UI·문구·버튼·동작 변경을 위한 1회성 커스텀 상품입니다.', features:['문구 변경','버튼/임베드 수정','소규모 UI 변경','기존 기능 내 간단 동작 수정','주문 전 범위 확인'] },
  { id:'vexo-custom-standard', name:'VEXO 중급 커스텀 옵션', category:'커스텀', price:34900, badge:'STANDARD', description:'기존 기능을 조합하거나 소규모 신규 자동화를 추가하는 1회성 커스텀 상품입니다.', features:['기능 조합/확장','소규모 신규 자동화','관리자 버튼·패널 추가','기존 데이터 구조 연동','주문 전 범위 확정'] },
  { id:'vexo-custom-large', name:'VEXO 대형 커스텀 옵션', category:'커스텀', price:69900, badge:'LARGE', description:'기존 VEXO 시스템에 비교적 큰 신규 기능을 제작하는 1회성 커스텀 상품입니다.', features:['대형 기능 추가','복수 기능 연동','고급 관리자 흐름','별도 테스트·검수','주문 전 상세 견적 및 범위 확정'] },
  { id:'vexo-major-upgrade', name:'VEXO 메이저 버전 업그레이드', category:'업그레이드', price:19900, badge:'VERSION', description:'VEXO의 새로운 메이저 버전이 출시될 때 기존 구매자가 해당 버전으로 업그레이드하는 영구 업그레이드 상품입니다.', features:['메이저 버전 1회 업그레이드','출시 버전별 적용 범위 공지','기존 구매 정보 확인','업그레이드 안내 제공','기간 제한 없음'] },
  { id:'vexo-seller-pack', name:'VEXO SELLER PACK', category:'패키지', price:44900, badge:'SELLER', description:'디스코드에서 실제 판매를 운영하는 데 필요한 핵심 기능을 한 번에 묶은 영구 패키지입니다.', features:['티켓 자동화 확장팩','결제 자동화 확장팩','통계 대시보드팩','운영 보조 기능','판매 운영용 관리자 구성','1회 구매 후 영구 적용'] },
  { id:'vexo-seller-pro', name:'VEXO SELLER PRO', category:'패키지', price:69900, badge:'PRO SELLER', description:'SELLER PACK에 고급 디자인과 보안·운영 옵션을 더한 상위 영구 패키지입니다.', features:['SELLER PACK 구성 포함','보안 강화팩','VEXO DESIGN BUNDLE','고급 운영 UI 구성','브랜딩 일괄 적용','1회 구매 후 영구 적용'] },
...
// VEXO 3.3 permanent add-ons: focused seller-operation upgrades, one-time purchase.
const VEXO_33_PRODUCTS = [
  { id:'vexo-customer-management', name:'VEXO 고객관리 확장팩', category:'봇 옵션', price:19900, badge:'CRM', description:'구매자·주문·후속 응대를 더 편하게 관리하기 위한 영구 고객관리 확장팩입니다.', features:['구매자별 주문 이력 확인','고객 메모/응대 정보 정리','재구매 고객 확인 보조','1회 구매 후 영구 적용'] },
  { id:'vexo-backup-restore', name:'VEXO 백업·복구 강화팩', category:'봇 옵션', price:12900, badge:'BACKUP', description:'판매 데이터와 운영 기록을 더 안전하게 보관하고 복구하기 위한 영구 백업 강화팩입니다.', features:['주기적 데이터 백업 강화','백업 파일 보존 관리','복구 절차 안내','운영 기록 보호 보조','1회 구매 후 영구 적용'] },
];
const existing33 = new Set(db.products.map(p => p.id));
for (const p of VEXO_33_PRODUCTS) {
  if (!existing33.has(p.id)) db.products.push(p);
}

// VEXO 3.5 permanent store-operation products. These are one-time add-ons.
const VEXO_35_PRODUCTS = [
  { id:'vexo-coupon-promo', name:'VEXO 쿠폰 & 프로모션 팩', category:'봇 옵션', price:14900, badge:'COUPON', description:'할인 코드와 프로모션 운영을 편하게 만드는 영구 상점 운영 확장팩입니다.', features:['퍼센트/정액 쿠폰 운영','사용기간·사용횟수 제한','최소주문금액 설정','관리자 쿠폰 발급·회수','1회 구매 후 영구 적용'] },
  { id:'vexo-sales-analytics', name:'VEXO 주문·매출 분석팩', category:'봇 옵션', price:19900, badge:'ANALYTICS', description:'주문·매출·상품 판매 흐름을 한눈에 볼 수 있도록 운영 통계를 확장하는 영구 옵션입니다.', features:['일/월 매출 분석','완료 주문 통계','상품별 판매량','평균 객단가','1회 구매 후 영구 적용'] },
  { id:'vexo-advanced-ticket', name:'VEXO 고급 티켓팩', category:'봇 옵션', price:19900, badge:'TICKET PRO', description:'주문 티켓을 더 깔끔하고 빠르게 운영할 수 있도록 고급 티켓 UI와 처리 흐름을 추가하는 영구 옵션입니다.', features:['고급 주문 요약 카드','처리 상태 타임라인','담당자/상태 표시','처리 버튼 정리','완료 화면 강화','1회 구매 후 영구 적용'] }
];
const existing35 = new Set(db.products.map(p => p.id));
for (const product of VEXO_35_PRODUCTS) {
...
// alerts all look like one consistent VEXOHUB notification feed in Discord.
function buildOrderEmbed(order, { emoji, title, color, extraFields = [], footer } = {}) {
  ...
  return {
    ...
    embeds: [{
      ...
      color,
      fields,
      footer: { text: footer || 'VEXOHUB · 웹사이트 자동 알림' },
...
function appendSystemMessage(order, text, now = new Date().toISOString()) {
  const message = { id:'SYS-' + nanoid(10).toUpperCase(), senderId:'system', senderName:'VEXOHUB', senderRole:'system', text, attachment:null, createdAt:now };
  order.messages.push(message);
  return message;
}

function transitionOrder(order, target, actorId, actorName = 'VEXOHUB') {
...
async function sendDiscordCompletionNotice(order) {
  ...
  await postWebhook(buildOrderEmbed(order, {
    ...
    color: 0x22c55e,
    footer: 'VEXOHUB · 지급완료 매출은 관리자 대시보드 통계에도 반영됩니다.',
  }));
}
...
app.get('/api/admin/customers', requireAdmin, (req, res) => {
  ...
  const customers = [...map.values()].sort((a,b) => String(b.lastOrderAt).localeCompare(String(a.lastOrderAt)));
  res.json({ customers });
});
...
app.listen(PORT, () => console.log(`VEXOHUB running on http://localhost:${PORT}`));
